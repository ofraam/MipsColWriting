...
'''
This class represents a single user session, which includes a list of actions (edits)
'''
class Session:
...
'''
This class represents a single action (paragraph edit)
@user: the team member who made the edit
@ao: the object that was modified
@actType: was the object added/modified/deleted
@weightInc: by how much this edit should increment weight
@changeExtent: by how mach the paragraph changed (cosine similarity)
'''
class Action:
...
class Mip:
    def __init__(self, firstVersion):
        self.mip = nx.Graph() #the network itself
        self.pars = []  # a list of the paragraph ids
        self.users = {}  # the users (team members)
        self.nodeIdsToUsers = {}  # dictionary to map between node ids in the network and user ids
        self.latestVersion = 0  # number of last version of the document
        self.lastID = 0  # the last id used in the network (to know which id to assign next)
        self.decay = 0.01  # rate of decaying weights when paragraphs are not edited
        self.sigIncrement = 1  # by how much to increment the weight between two paragraphs that were both edited substantiall
        self.minIncrement = 0.1  # by how much to increment the weight between two paragraphs where one or both had minor edit
        self.currentVersion = firstVersion  # current version of the document
        self.current_flow_betweeness = None  # centrality values of all nodes
        self.log = [] # log holds all the session data (list of sessions)
    ...
    def initializeMIP(self):  # initialize a MIP-net
        self.pars.append({})
...
#        print self.mip.nodes(True)
    ...
    def updateMIP(self, newVersion):  # update the network based on the new version of the document
...
#                    print 'not decaying'
        ...
        self.log.append(session)
        try:  # update centrality of nodes
...
#            print self.mip.node[self.lastID]
    ...
    def getLiveObjects(self):  # returns paragraphs that have not been deleted so far
        liveObjects = []
    ...
    #    print api_obj
        ...
        # compute proximity between user node and object node using the specified similarity metric
        proximity = 0
...
#        print 'proximity = '+str(proximity)
        return alpha*api_obj+beta*proximity # returns DOI computation
...
#        print 'prob' + str(prob)
    ...
    '''
    rank all live objects based on DOI to predict what edits a user will make (this is the MIP-DOI algorithm described
    in the paper)
...
#                        doi = checkedObjects[act.ao] #already computed doi, don't recompute!
    ...
    def rankChangesGivenUserFocus(self,user,focus_obj, time):  #MIP-DOI if we know the focus object (not the case with Wikipedia data)
        notificationsList = []
        checkedObjects = []
...
def get_pickle(pickle_file, folder="pickles"):  # gets data from pickle file
...
def lev_sim(a, b):  # Levenshtein similarity between paragraphs (used for matching paragraphs across revisions)
    return Levenshtein.ratio(a, b)

def cosine_sim(a, b):  # cosine similarity between paragraphs (used to compute the extent of change to a paragraph)
    if ((len(a)<5) | (len(b)<5)):
        return 1
...
def generate_ratios(paras1, paras2, reverse=False, sim_func=lev_sim):  # this is for matching paragraphs
    '''
...
def evaluateGeneralParagraphRankingForAuthors(articleRevisions, articleName):
    ...
    last_author_revs = {}
    author_rev_counts = {}
    author_rev_counts[''] = 1 # always true for anonymous editors
    prev_author = None
...
#    for i in range(len(articleRevisions.revisions)):
    results = []
    for i in range(2,len(articleRevisions.revisions)):  # iterate over revisions
        j=0
        print i
...
#            print 'strange'
        if ((cur_author in last_author_revs) & (cur_author!="")) : #check that there was a previous revision for this author (otherwise can't compute DOI, though could do just api, maybe add later), and that the author is not anonymous (have no info)
            if last_author_revs[cur_author]<i-1: #if the author wrote the previous revision, nothing to do.
                ...
                # compute MIP-DOI using different alpha and beta weights
        ...
        else: # either anonymous author, or first time author. In these cases no point of doing doi, but can still do api (alpha = 1)
        ...
        if ((i>0) & ((prev_author!=cur_author) | (cur_author == ""))):
            actualEditsFull = getSignificantEditsOfAuthor(cur_author, i, i+j, articleName)
...
def runEvalOnArticle(pickle_file_name,generateMIPs = False):
        ...
        #    results = evaluateChangesForAuthors(current_pickle,"johann_pachelbel")
            ...
            resFileName = "author_predictions_Dec20/"+ pickle_file_name[:-4] + "_live_par_predictions_sigOnly.csv"
            writeResultsToFile(results, resFileName)
            
            results = evaluateChangesForAuthors(current_pickle,pickle_file_name[:-4])
            resFileName = "author_predictions_Dec20/"+ pickle_file_name[:-4] + "_changes_predictions_sigOnly.csv"
            writeResultsToFile(results, resFileName)
        except:
            print "Unexpected error:", sys.exc_info()[0]   
            traceback.print_exc() 
...
if __name__ == '__main__':
    # runEvalOnFolder('runPickles', False)  # runs evaluation on all the articles in the folder
    runEvalOnArticle('Film_noir.pkl', False)  # run evaluation on specific article
...
#        print str(change[1])
    ...
    # an example of how to generate a plot of a MIP-Net (uncomment to try
#     mip = readMIPfromFile("Netherlandish", 15)  # reads a previously stored network
#     edgewidth=[]
